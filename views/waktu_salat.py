...
with tab1:
    ...
    st.info("Klik tombol di bawah untuk mengambil lokasi dari GPS browser Anda.")

    if st.button("📍 Ambil Lokasi GPS"):

        # Jalankan JavaScript di browser untuk ambil GPS
        ...
        if coords and isinstance(coords, dict) and "lat" in coords:
            st.session_state.location["lat"] = coords["lat"]
            st.session_state.location["lon"] = coords["lon"]
            st.session_state.location["tz"] = get_timezone(coords["lat"], coords["lon"])
            st.success(f"✅ Lokasi GPS berhasil: {coords['lat']:.5f}, {coords['lon']:.5f}")
            st.rerun()
        elif coords is not None:
            st.warning("⚠️ GPS gagal. Pastikan izin lokasi diaktifkan di browser.")
